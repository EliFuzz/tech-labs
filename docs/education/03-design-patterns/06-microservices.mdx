        ...
        <Tabs>
            <TabItem value="data" label="Data Patterns" attributes={{className:"tabs__vertical"}}>
              ...
              **Principles**
              ...
              - **Database should be private**: inaccessible by other services
              - **Keep data private**
                  - **Private tables per service**: each service owns specific tables (Lowest overhead)
                  - **Schema per service**: separate database schema for each service (clear ownership)
                  - **Database server per service**: for high-throughput scenarios
              - **Enforce data privacy**: use separate database users and access controls

              **Pros**

              - **Loose Coupling**: Changes in one service's database don't affect others
              - **Flexibility**: Services can use the best database type for their needs (e.g., ElasticSearch for search, graph database for social graphs)

              **Cons**

              - **Distributed Transactions**: Complex and often unsupported by NoSQL databases (CAP theorem)
              - **Multi-service Queries**: Joining data across databases is challenging
              - **Management Complexity**: Managing multiple databases (SQL and NoSQL) can be complex

              **Suggestions**

              - **Transactions**: Use the Saga pattern for transactions spanning services
              - **Multi-service queries**
                  - **API Composition**: Application performs joins instead of the database
                  - **CQRS**: Maintain materialized views joining data from different services, updated through event subscriptions
              ...
              **Pros**
              ...
              - Easy data consistency with familiar transactions
              - Simpler to manage one database

              **Cons**

              - Slower development due to coordination for schema changes
              - Potential performance issues from services interfering with each other
              - Database may not meet all service needs
              ...
              **Pros**
              ...
              - **Simple Data Queries**: Each service owns its data, making queries straightforward within a service

              **Cons**

              - **Inefficient Joins**: Complex queries requiring data from multiple services might involve in-memory joins of large datasets, leading to performance issues
              ...
              **Pros**
              ...
              - **Scalability and Performance**: Supports multiple, denormalized views optimized for specific queries, leading to better performance and scalability
              - **Separation of Concerns**: Clear separation between command and query models simplifies development and maintenance
              - **Event Sourcing**: A natural fit for event-sourced architectures, where events are the source of truth for data updates

              **Cons**

              - **Complexity**: Implementing and managing separate models and data flows can increase overall system complexity
              - **Code Duplication**: Denormalized views might lead to some data redundancy and potential for code duplication
              - **Eventual Consistency**: Due to asynchronous updates, views may not reflect the latest changes immediately (eventual consistency). This requires careful handling for scenarios demanding strict real-time data accuracy
              ...
              **Pros**
              ...
              - **Data Consistency**: Maintains data consistency across distributed services without complex distributed transactions

              **Cons**

              - **Complexity**: Implementing compensating transactions and handling message delivery issues increases development complexity
              - **Atomic Updates**: Requires careful design to ensure updates to the database and message publication happen atomically
              - **Client Outcome**: Different approaches for handling client notification of saga completion have trade-offs
                - **Long-Polling**: Client polls for updates, leading to potential inefficiency
                - **Event-Driven Notification**: Requires additional infrastructure for event delivery, but offers better responsiveness
                - **Initial Response with Polling**: Balances initial response with potential polling overhead
              ...
              **Pros**
              ...
              - Enables reliable event publishing in event-driven architectures
              - Avoids object-relational impedance mismatch
              - Provides complete audit history of data changes
              - Allows querying past states of entities (time travel)
              - Simplifies migration to microservices due to loosely coupled entities

              **Cons**

              - Complex querying due to need to reconstruct entity state (Often requires CQRS and eventual consistency)
